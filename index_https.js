...
app.get("/random", async (req, res) => {
    try {
        ...
        const studios = show.studios ? show.studios.map(studio => ({ name: studio.name, url: studio.url })) : [];
        ...
        res.render("random", {
            ...
            score: show.score,
            studio: studios,
            studio_url: studios.map(studio => studio.url).join(', ') || ''
        });
    } catch (error) {
...
app.get("/search", async (req, res) => {
    ...
    try {
        ...
        const animeList = result.data.map(show => ({
            ...
            rating: show.rating,
            synopsis: show.synopsis,
            genres: show.genres ? show.genres.map(genre => genre.name) : [],
            episode: show.episodes,
            status: show.status,
            score: show.score,
            studios: show.studios ? show.studios.map(studio => ({ name: studio.name, url: studio.url })) : [],
        }));
...
app.get("/anime/:id", async (req, res) => {
    ...
    try {
        ...
        console.error(`Error fetching anime with ID ${animeID}:`, error.message);
        res.status(500).send("Internal Server Error");
    }
});
